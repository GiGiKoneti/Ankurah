    ...
    function connect() {
      ...
      es.onmessage = (e) => {
        try {
          ...
          console.log('[SSE EVENT]', data) // Diagnostics for the user

          if (data.type === 'alert') {
            setLatestAlert(prev => {
              // Preserve snapshot_url if the camera matches
              const existingUrl = (prev && prev.camera_id === data.camera_id) ? prev.snapshot_url : null
              return { ...data, snapshot_url: existingUrl }
            })
            setAlerts(prev => [data, ...prev].slice(0, 50))
          } else if (data.type === 'snapshot_fired') {
            setLatestAlert(prev => {
              if (prev && prev.camera_id === data.camera_id) {
                return { ...prev, snapshot_url: data.snapshot_url }
              }
              // Hybrid: data from snapshot event + defaults
              return { 
                camera_id: data.camera_id, 
                snapshot_url: data.snapshot_url, 
                timestamp: new Date().toISOString(),
                location_name: "Live Feed — Detect",
                confidence: 1.0, 
                type: 'alert' 
              }
            })
